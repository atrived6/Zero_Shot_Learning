...
class DataLoader_AWA2:
    ...
    def getAttrs(self):
        attrSplit = sio.loadmat(self.path + '/' + self.dataset + '/' + 'att_splits.mat')
        att = attrSplit['original_att']
        att = np.transpose(att)
        return att

    def getLabels(self):
        res101 = sio.loadmat(self.path + '/' + self.dataset + '/' + 'res101.mat')
        data = res101['labels']
        return data
    ...
    def saveWordVecsAndAttrVecsOrig(self):
        ...
        Labels = np.array(Labels)
        # pdb.set_trace()
        fDst = hp.File(self.path + + '/' + self.dataset + '/' + 'embeddingsOrig.h5', 'w')
        fDst.create_dataset("word2Vecs", data=tosaveWordVecs)
        fDst.create_dataset("attrVecs", data=tosaveAttrVecs)
        fDst.create_dataset("Labels", data=Labels)
        fDst.close()
    ...
    def saveToFile(self, file, data):
        ...
        att = np.reshape(np.array(att), (len(att), 64))
        ...
        fDst.close()
        print("Made " + self.path + '/' + self.dataset + '/' + file)

    def makeLocData(self):
        mat = sio.loadmat(self.path + '/' + self.dataset + '/' + 'att_splits.mat')
    ...
    def makeData(self):
        ...
        labelsTrain1 = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'trainclasses1.txt')
        labelsTrain2 = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'trainclasses2.txt')
        labelsTrain3 = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'trainclasses3.txt')
        labelsVal1 = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'valclasses1.txt')
        labelsVal2 = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'valclasses2.txt')
        labelsVal3 = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'valclasses3.txt')
        labelsTest = self.getLabelsFromTxt(file=self.path + '/' + self.dataset + '/' + 'testclasses.txt')
	...
	w2vMat = sio.loadmat(self.path + "/" + self.dataset + '/word2vecs.mat')
	self.word2VecsPerClass = w2vMat['word2vecs']
        self.exclude = w2vMat['exclude']
        #newClassNames = w2vMat['newClassNames']
        #self.word2VecsPerClass, self.exclude = self.getWord2VecsGlove(self.path + '/' + self.dataset + '/' + 'allClasses.txt')
...
if __name__ == '__main__':
    ...
    path = './data'
    dataset = 'APY'
    dataloader = DataLoader_AWA2(path, dataset)
    dataloader.makeData() 
